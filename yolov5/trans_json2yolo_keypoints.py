...
def translate_info(file_names: list, save_root: str, class_list: list, keypoints_list: list, train_val='train'):
    """
    ...
    for file in tqdm(file_names, desc="translate {} file...".format(train_val)):
        ...
        with open(os.path.join(save_txt_path, file + ".txt"), "w") as f:
            ...
            for index, obj in enumerate(data["shapes"]):
                if obj['shape_type'] == "rectangle" and obj["label"] in class_list:
                    ...
                    str_list = [class_index, xcenter, ycenter, w, h]
                    point_list = [0.0 for _ in range(len(keypoints_list)*2)]
                    point_num = 0
                    for ann in data["shapes"]:
                        if ann['shape_type'] == "point":
                            if float(ann["points"][0][0]) > xmin and float(ann["points"][0][0]) < xmax and float(ann["points"][0][1]) < ymax and float(ann["points"][0][1]) > ymin:
                                point_x = float(ann["points"][0][0])
                                point_y = float(ann["points"][0][1])
                                point_x = round(point_x / img_width, 6)
                                point_y = round(point_y / img_height, 6)
                                if ann["label"] not in keypoints_list:
                                    print("{}不在keypoints_list中".format(ann["label"]))
                                    exit()
                                index = keypoints_list.index(ann["label"])
                                point_list[index * 2] = point_x
                                point_list[index * 2 + 1] = point_y
                    ...
                    if len(info) != 5 + len(keypoints_list)*2:
                        print(f'{file}生成的yolo格式长度不等于{5 + len(keypoints_list)*2}')
                        exit()
                    f.write(" ".join(info) + "\n")
...
if __name__ == "__main__":
    ...
    # voc数据集根目录以及版本
    voc_root = r"D:\MyData\data_flydetection\shenyang_08\voc_wentie_pose_data\VOCdevkit2007"
    voc_version = "VOC2007"
    ...
    # 转换后的文件保存目录
    save_file_root = r"D:\MyData\data_flydetection\shenyang_08\yolo_wentie_pose_data"

    # 数据集对应的类别名称，该名称要与yolov5中的yaml文件中的类别一致
    class_list = ['wentie']
    keypoints_list = ['zuoshang', 'youshang', 'youxia', 'zuoxia']
    kpt_shape = [len(keypoints_list), 2]
    ...
    translate_info(train_file_names, save_file_root, class_list, keypoints_list, "train")

    # 读取val.txt中的所有行信息，删除空行
    with open(val_txt_path, "r") as r:
        val_file_names = [i for i in r.read().splitlines() if len(i.strip()) > 0]
    # voc信息转yolo，并将图像文件复制到相应文件夹
    translate_info(val_file_names, save_file_root, class_list, keypoints_list, "val")
    # 生成yaml文件
    config_yaml = {"path": save_file_root, "train": 'train', "val": 'val', "kpt_shape": kpt_shape, "nc": len(class_list), "names": class_list}
